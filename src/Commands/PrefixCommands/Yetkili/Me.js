...
formattedChannels += `${client.emojis.cache.find(x => x.name == "tornado_isaret2")} \`${channelNameWithSpaces}:\` (**${channel.messageCount} Mesaj**)\n`;
});
...
const formattedTopVoiceChannels = topVoiceChannels.map(({ channel, time }, index) => {
    ...
    return `${client.emojis.cache.find(x => x.name == "tornado_isaret2")} \`${channelNameWithSpaces}:\` (**${time} Dakika**)\n`;
}).join('');
...
},
{
    ...
    value: `${formattedTopVoiceChannels ? formattedTopVoiceChannels : "\`Veri Bulunamıyor...\`"}
${client.emojis.cache.find(x => x.name == "tornado_isaret")} **\`Yakında büyük bir güncelleme gelicek hazırlıklı olun!\`**`,
    inline: false
}
)
